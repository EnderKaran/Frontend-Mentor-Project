...
export default function CommentCard({ item, currentUser }: CommentCardProps) {
  ...
  // Avatar resmini kullanıcı adına göre al
  const getUserAvatar = (username: string) => {
    return avatars[username as AvatarKey] || avatars.juliusomo;
  };

  return (
    <div className="relative flex flex-col gap-4 p-4 bg-white rounded-lg shadow-sm md:p-6 md:flex-row md:gap-6">
      ...
      <div className="flex-1 order-1 md:order-2">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4">
            <img 
              src={getUserAvatar(item.user.username)} 
      ...
      <div className="absolute flex items-center gap-4 md:hidden bottom-6 right-4">
        {isCurrentUser ? (
           <>
           <button className="flex items-center gap-2 text-[#ed6368] font-bold">
             <img src={iconDelete} alt="" /> Delete
           </button>
